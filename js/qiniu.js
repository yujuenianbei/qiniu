...
;(function( global ){
    document.write("<script src='../js/plupload.full.min.js'></script>");
    document.write("<script src='../js/plupload.min.js'></script>");
/**
    ...
    QN.imgs[id] = Qiniu.uploader({
        ...
        init: {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    function showPreview (file) {
                        ...
                        preloader.onload = function() {
                            $('#'+id).css({ 'background':'url('+preloader.getAsDataURL()+')','background-size':'cover' });
                        };
            ...
            'UploadProgress' : function (up){
                if(QN.uploadType){
                    QN.uploadType(id,up.total.percent);
                }
            },
            'FileUploaded': function(up, file, info) {
                ...
                // if(QN.uploadEnd){
                    ...
                    console.log(sourceLink);//图片的完整地址
                    console.log(domain);//图片的domain（七牛服务器的地址）
                    console.log(res.key);//图片名称
                    mm(sourceLink)
                // }
            },
            'Error': function(up, err, errTip) {
                ...
                // mm(id+'上传出错');
                if(QN.uploadError)
                    QN.uploadError(id,err,errTip)
            }
        }
    });
}

QN.domain=$('[data-domain]').attr('data-domain');
QN.uptoken=$('[data-uptoken]').attr('data-uptoken');
QN.imgs={}
...
// // 预览图片
// QN.showImg=function(tag,img,fn){
//     // 图片格式
//     if(['.jpg','.JPG','.png','.PNG'].indexOf(/.[^.]+$/.exec(tag.value)[0])==-1)return
...
// 裁剪
QN.crop=function (w,h,x,y){
    return Qiniu.imageMogr2({
...
// 水印
// watermark/1/image/aHR0cDovL29nMmE5Ymh0MC5ia3QuY2xvdWRkbi5jb20vb18xYjY1bW92MnY5MGQxdHE2dm1jMWIzZzZ0dTkucG5n/dissolve/100/gravity/Center/ws/1
// 缩略图
// imageView2/1/w/200/h/200/format/jpg
